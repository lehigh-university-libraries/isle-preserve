...
final class UsageReport {
  ...
  /**
   * Aggregates usage before looking up media parents and work metadata.
   */
  public function events(array $criteria, ?array $period = NULL, bool $aggregate = FALSE, bool $geography = FALSE): SelectInterface {
    ...
    foreach (['node', 'media'] as $type) {
      ...
      if ($aggregate) {
        ...
        if ($geography) {
          // Country reports do not need separate rows for each city/region.
          $branch->leftJoin('entity_metrics_regions', 'r', 'r.id = d.region_id');
          $branch->addExpression("COALESCE(NULLIF(UPPER(TRIM(r.country)), ''), 'Unknown')", 'country_code');
          $branch->groupBy('country_code');
        }
      }
      else {
      ...
      if ($type === 'media') {
        if ($aggregate) {
          // Materialize the selected usage bucket, not every media relationship.
          $branch = $this->database->select($branch, 'd');
          $branch->fields('d', ['entity_type', 'entity_id']);
          // Expressions follow fields in SQL: keep node/media UNION aligned.
          foreach (['event_count', 'first_event', 'last_event'] as $column) {
            $branch->addExpression('d.' . $column, $column);
          }
          if ($geography) {
            $branch->addExpression('d.country_code', 'country_code');
          }
        }
        ...
        // Repeated references must not multiply an event or an entity total.
        $branch->distinct();
      }
      else {
