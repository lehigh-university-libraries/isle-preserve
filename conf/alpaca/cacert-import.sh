...
if [ ! -f "/usr/local/share/ca-certificates/lehigh.pem" ]; then
  exit 0
fi

if [[ -s "/usr/local/share/ca-certificates/lehigh.pem" ]]; then
    if hash keytool &>/dev/null; then
        keytool \
            -importcert \
            -noprompt \
            -cacerts \
            -storepass changeit \
            -file "/usr/local/share/ca-certificates/lehigh.pem" \
            -alias lehigh.pem
    fi
fi
